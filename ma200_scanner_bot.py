...
def scan_symbol(symbol, btc_bullish=True):
    ...
    for interval, tf in [("4h","4H"), ("1d","1D")]:
        ...
        vol_ok = vols[-1] > avg_v * 1.2   # FIX: harus 1.2x average, bukan cuma > average
        ...
        # FIX 4: Skip sinyal 1D altcoin kalau BTC bearish
        if tf == "1D" and not btc_bullish:
            continue
        ...
        if cl <= ma200c * 1.005 and cc > ma200c and body > 1.0:
