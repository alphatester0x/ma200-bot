...
fetch_ohlcv sekarang return highs yang benar (field index [2] dari klines).
"""

from typing import Optional, Tuple, List
from session import SESSION
from config import BINANCE_BASE, SKIP_SYMBOLS, SKIP_SUBSTRINGS, MIN_VOL_USDT
...
OHLCVData = Tuple[
    List[float],  # opens
    List[float],  # highs   ← BARU: real high dari klines, bukan derived
    List[float],  # lows
    List[float],  # closes
    List[float],  # volumes
]
...
def fetch_ohlcv(symbol: str, interval: str) -> Optional[OHLCVData]:
    """
    Ambil OHLCV data dengan real high (index [2]).
    limit=202 — cukup untuk MA200 + 1 bar lookback.
    ...
    try:
        ...
        return (
            ...
            [float(c[2]) for c in raw],  # high  ← real high, bukan max(o,c)
...
def fetch_24h_vol_usdt(symbol: str) -> float:
    """
    Ambil volume 24h dalam USDT via klines 1d (1 candle).
    Digunakan untuk filter likuiditas minimum.
    Returns 0.0 jika gagal.
    """
    try:
        resp = SESSION.get(
            f"{BINANCE_BASE}/api/v3/klines",
        ...
        if resp.status_code == 200:
            raw = resp.json()
            if raw:
                # index [7] = quoteAssetVolume (volume dalam USDT)
                return float(raw[0][7])
    except Exception:
        pass
    return 0.0
